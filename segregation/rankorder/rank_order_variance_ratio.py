...
from ..singlegroup import CorrelationR
from ._utils import (
    _core_data,
    _rank_order,
...
def _rank_order_variance_ratio(data, groups, degree=4):
    """Calculate the Rank-Order Variance Ratio Index.
    ...
    ----------
    statistic : float
        Rank-order variance ratio index statistic value
    ...
    Notes
    -----
    Reardon's :math:`R^R`, the variance ratio (correlation ratio) index
    integrated across the thresholds of an ordered variable using the weight
    :math:`I(p) = 4p(1 - p)`. Of the three rank-order indices this one weights
    thresholds most evenly across the distribution.

    The pairwise index is :class:`segregation.singlegroup.CorrelationR`, which
    is algebraically identical to the variance ratio
    :math:`\\sum_j t_j (p_j - P)^2 / (T P (1 - P))`.
    ...
    statistic, profile, deltas, model = _rank_order(
        data, groups, degree, CorrelationR, variance_ratio_weight, deltas_variance_ratio
    )
    return statistic, _core_data(data, groups), groups, profile, deltas, model


class RankOrderVarianceRatio(RankOrderIndex, SpatialImplicitIndex):
    """Rank-Order Variance Ratio Index.
    ...
    See Also
    --------
    segregation.singlegroup.CorrelationR : the pairwise index integrated here.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        aux = _rank_order_variance_ratio(self.data, self.groups, degree)
        ...
        self._function = _rank_order_variance_ratio
